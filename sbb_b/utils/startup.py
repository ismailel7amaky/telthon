...
async def saves():
    try:
        ...
        print(str(e))
    try:
        await sbb_b(JoinChannelRequest("@GR_OUP_1 "))
    except BotMethodInvalidError:
        pass
    except ChannelsTooMuchError:
        LOGS.info("انضم بجروب السورس  اولا @GR_OUP_1")
    except ChannelPrivateError:
        LOGS.critical(
            "تم حظرك من استخدام سورس السلطان  عليك الأعتذار الى مطور السورس @U_U_Q_1"
        )
    try:
        await sbb_b(JoinChannelRequest("@U_U_Q_1"))
    except BaseException:
        pass
    try:
        await sbb_b(JoinChannelRequest("@C_U_U_1"))
    except BaseException:
        pass
    try:
        await sbb_b(JoinChannelRequest("@C_U_U_1"))
    ...
    try:
        if BOTLOG:
            Config.JMTHONLOGO = await sbb_b.tgbot.send_file(
                BOTLOG_CHATID,
                "https://telegra.ph/file/d0470c8c6ccfb03cfa9d1.mp4",
                caption="**تم تشغيل سورس السلطان بنجاح لعرض الاوامر ارسل .الاوامر**",
                buttons=[(Button.url("كروب المساعدة", "https://t.me/gr_oup_1"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        photobt = await sbb_b.upload_file(file="razan/pic/Elsoltan.jpg")
