...
def main_menu():
    text = f"**▾∮ مرحبا عزيزي {mention}**\n**▾اليكَ قائمة بازرار مضمنة لاوامر ↫**⍣ⵧⵧⵧ 𝘴ꪮꪊ𝘳ᥴꫀⵧⵧⵧⵧ⍣**\n[ََِِ ًًٍٍ𝗨ٍّّ𝘀ََِِ𝗲ًًٍٍ𝗥ًًٍٍ𝗕ُُ𝗼ٖٔ𝗧 🧸♥](https://t.me/C_U_U_1)\n\n"
    buttons = [
...
async def inline_handler(event):
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("**sbb_b"):
            ...
            result = builder.article(
                title="sbb_b Help™",
                description="**▾∮ قائمة التعليمات الخاصة ب ببوت السلطان **",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
        ...
        buttons = [
            (
                Button.url("قناة السورس", "t.me/C_U_U_1"),
                Button.url(
                    "المطور",
                    "t.me/U_U_Q_1",
                ),
        ...
        text, msg_entities = await event.client._parse_message_text(
            "لجعل بوت السلطان من نصيبك!", "md"
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
            type="photo",
            title="[ 𝘴ꪮꪊ𝘳ᥴꫀ♥](https://t.me/C_U_U_1)",
