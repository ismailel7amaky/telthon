...
async def icalc(e):
    if e.client._bot:
        return await e.reply("**الحاسبة العلمية لسورس السلطان\n @C_U_U_1**", buttons=lst)
...
async def inlinecalc(event):
    ...
    if (
        ...
        calc = event.builder.article(
            "Calc", text="**الحاسبة العلمية لسورس السلطان\n @C_U_U_1**", buttons=lst
        )
...
async def _(e):
    ...
    lst = list(zip(tultd[::4], tultd[1::4], tultd[2::4], tultd[3::4]))
    lst.append([Button.inline("=", data="calc=")])
    await e.edit("**الحاسبة العلمية لسورس السلطان\n @C_U_U_1**", buttons=lst)
